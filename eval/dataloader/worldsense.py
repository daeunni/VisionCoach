...
class WorldSense_Bench:
    ...
    def get_data(self):
        ...
        self.docs = []
        all_docs = []
        filename_tsv = os.path.join(self.data_dir, 'WorldSense.tsv')
        filename_json = os.path.join(self.data_dir, 'worldsense_qa.json')
        if os.path.isfile(filename_tsv):
            self.docs.append(pd.read_csv(filename_tsv, sep="\t"))
            print(f"Loaded TSV annotations: {filename_tsv}")
        elif os.path.isfile(filename_json):
            json_rows = self._load_docs_from_json(filename_json)
            self.docs.append(pd.DataFrame(json_rows))
            print(f"Loaded JSON annotations: {filename_json}")
        else:
            raise FileNotFoundError(
                f"Neither '{filename_tsv}' nor '{filename_json}' exists."
            )
    ...
    def _load_docs_from_json(self, json_path):
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        rows = []
        if isinstance(data, dict):
            for vid, item in data.items():
                task = item.get("task0", {}) if isinstance(item, dict) else {}
                rows.append(
                    {
                        "video": item.get("video_id", vid),
                        "question": task.get("question", ""),
                        "candidates": str(task.get("candidates", [])),
                ...
                rows.append(
                    {
                        ...
                        "candidates": str(item.get("candidates", item.get("options", []))),
    ...
    def _resolve_video_path(self, video_id):
        direct = osp.join(self.data_dir, f"{video_id}.mp4")
        if osp.exists(direct):
            return direct
        if not hasattr(self, "_video_index"):
            self._video_index = {}
            for p in glob(osp.join(self.data_dir, "worldsense_videos_*", "*.mp4")):
                self._video_index[osp.splitext(osp.basename(p))[0]] = p
        return self._video_index.get(video_id, direct)
    ...
    def process_data(self, line):
        ...
        if isinstance(candidates_raw, str):
            try:
                candidates = eval(candidates_raw)
            except Exception:
...
def worldsense_process_result(eval_file, **judge_kwargs):
        ...
        # Some generated result files do not contain an explicit "index" column.
        # Compute score per row directly so evaluation is robust to column schema.
