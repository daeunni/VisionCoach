...
parser.add_argument('--gpu_memory_utilization', type=float, default=0.90,
                    help='Fraction of GPU memory to use (0~1). Lower if OOM, e.g. 0.85 or 0.90. Default 0.90.')
args = parser.parse_args()

result_file = args.result_file
model_name = args.model_path

print(f"Loading model with tensor_parallel_size={args.tensor_parallel_size}, gpu_memory_utilization={args.gpu_memory_utilization}...")
llm = LLM(
    model=model_name,
    tensor_parallel_size=args.tensor_parallel_size,
    trust_remote_code=True,
    max_model_len=4096,  # 평가에 충분한 길이
...
def build_prompt(question, gt, candidate):
    """Build a single prompt for evaluation."""
...
def evaluate_json(file_path):
    ...
    # Step 1: Build all prompts for batch processing
    print("Building prompts for batch evaluation...")
    prompts = []
    valid_indices = []  # Track which indices have valid data
    
    for idx, item in enumerate(data):
        if 'answer_vqa' in item and item['answer_vqa']:
            prompt = build_prompt(item['question'], item['answer'], item['answer_vqa'])
            prompts.append(prompt)
            valid_indices.append(idx)
    
    print(f"Total samples: {len(data)}, Valid samples for VQA evaluation: {len(prompts)}")
    
    # Step 2: Batch evaluate all prompts at once
    print("Running batch evaluation with vLLM...")
    all_scores = batch_evaluate(prompts)
    print(f"Batch evaluation complete! Processed {len(all_scores)} samples.")
    
    # Create a mapping from original index to score
    score_map = {valid_indices[i]: all_scores[i] for i in range(len(valid_indices))}

    # Step 3: Process results
    for idx, item in enumerate(tqdm(data, desc=f"Processing {model_name} results", unit="item")):
